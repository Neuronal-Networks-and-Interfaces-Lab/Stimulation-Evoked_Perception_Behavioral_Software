...
LayerTextArray = split(layer1,",");
for v = 1:length(LayerTextArray)
    LayerNumArray(v) = str2double(LayerTextArray(v));
end
...
elseif strcmpi(choice,'Layer')==1
    ...
    for w = 1:length(LayerNumArray)
        err = PS_SetRepetitions(StimN, LayerNumArray(w), NRep);
        switch err
            case 0
                disp('Number of pulses set:');
                disp(NRep);
        ...
        err = PS_SetRate(StimN, LayerNumArray(w), Rate);
        switch err
            case 0
                disp('Frequency set:');
                disp(Rate);
        ...
        err = PS_SetRectParam(StimN, LayerNumArray(w), Param);
        switch err
            case 0
                disp('Stimulation biphasic pattern parameters have been set');
            otherwise
